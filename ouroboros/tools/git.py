...
def _check_prompt_integrity(ctx: ToolContext) -> Optional[str]:
    """Check that protected files are not modified or corrupted. Returns error string or None."""
    import subprocess
    # Check if any protected files are about to be committed
    try:
        result = subprocess.run(
            ["git", "diff", "--cached", "--name-only"],
            cwd=ctx.repo_dir, capture_output=True, text=True, timeout=10
        )
        staged = result.stdout.strip().split("\n") if result.stdout.strip() else []
        for f in staged:
            if f in PROTECTED_FILES:
                return f"⛔ PROTECTED_FILE: {f} is protected. Evolution cannot modify identity files."
    except Exception:
        pass
    # Check content integrity of prompt files
